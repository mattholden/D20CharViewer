...
public abstract class D20SRD implements D20Fantasy {
	...
	public static final class Feats { 
		...
		public static final D20Feat
			...
			MARTIAL_WEAPON_PROFICIENCY = new MartialWeaponProficiency(),
			...
			IRON_WILL = new IronWill(),
			LEADERSHIP = new Leadership(),
			LIGHTNING_REFLEXES = new LightningReflexes(),
			MAGICAL_APTITUDE = new MagicalAptitude(),
			MOUNTED_COMBAT = new MountedCombat(),
			MOUNTED_ARCHERY = new MountedArchery(),
			RIDE_BY_ATTACK = null,
			SPIRITED_CHARGE = null,
			TRAMPLE = null,
			NATURAL_SPELL = null,
			NEGOTIATOR = new Negotiator(),
			NIMBLE_FINGERS = new NimbleFingers(),
			PERSUASIVE = new Persuasive(),
			POINT_BLANK_SHOT = new PointBlankShot(),
			FAR_SHOT = new FarShot(),
			PRECISE_SHOT = new PreciseShot(),
			IMPROVED_PRECISE_SHOT = new ImprovedPreciseShot(),
			MANYSHOT = new Manyshot(),
			...
			HEIGHTEN_SPELL = null,
			MAXIMIZE_SPELL = null,
			QUICKEN_SPELL = new QuickenSpell(),
			SILENT_SPELL = null,
			STILL_SPELL = null,
			WIDEN_SPELL = null,
